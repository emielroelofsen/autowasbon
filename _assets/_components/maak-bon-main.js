...
import { SOAP_THEME, SOAP_THEMES, setMaakBonPage } from './carwash-config.js';
import { applyUVMappings } from './uv-mapping-config.js';
import { WasbonController } from './wasbon-controller.js';

// Override theme for wasbon maken page - use 'Start' theme
const WASBON_THEME = 'Start';
...
async function loadPendingStation(pending) {
  ...
  try {
    ...
    if (textureManager) {
      ...
      applyUVMappings(textureManager, WASBON_THEME, SOAP_THEMES, stations);
    }
...
async function loadAllStations() {
  ...
  // Get current theme - use WASBON_THEME for wasbon maken page
  const currentTheme = SOAP_THEMES[WASBON_THEME] || SOAP_THEMES['Start'];
  ...
  // Apply UV mappings - use WASBON_THEME for wasbon maken page
  applyUVMappings(textureManager, WASBON_THEME, SOAP_THEMES, stations);
...
function init() {
  ...
  // Initialize wasbon controller (for maak-bon.html)
  // Pass textureManager and stations so theme can be changed dynamically
  wasbonController = new WasbonController(cameraController, sceneSetup, textureManager, stations);
  window.wasbonController = wasbonController;
